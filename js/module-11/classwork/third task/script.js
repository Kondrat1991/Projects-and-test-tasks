...
let getResult = phones.reduce(function(acc, item,) {
    return acc + template(item)},"");

container.innerHTML = getResult;

function handleClickPreview(event) {
    if(event.target.classList.contains("module-card-button") ){
    let knock = document.querySelector(`#${event.target.dataset.id}`);
    console.log(knock);
    knock.classList.toggle("active");
    }
}
let div = document.querySelector(".container");
div = document.addEventListener("click", handleClickPreview);

let appleButton =  document.querySelector(".apple");

// let newArr = phones.filter( (elem)=> elem.categoryId === '1');
// console.log(newArr);

function getApple(phones) {
    let newArr = phones.filter( (elem)=> elem.categoryId === '1');
    console.log(newArr);
    container.innerHTML = newArr.reduce(function(acc, item,) {
        return acc + template(item)},"");
}
function handleAplleClick() {
    getApple(phones);
}

appleButton.addEventListener("click", handleAplleClick);
...
// function getApple(newArr) {
//
//
//     let getApple = newArr.reduce(function(acc, item,) {
//     //let htmlRes = Handlebars.compile(input);
//     return acc + template(item)},"");
//
//     container.innerHTML = getApple;
//
// }


//appleButton.document.addEventListener("click", getApple);




//По кліку на баттон показує тільки два айфона
